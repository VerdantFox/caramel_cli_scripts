...
def sample_folders(cases,  doc_count, host, port, user_name, password, thread_count, purge):
    ...
    for case in cases:
        ...
        try:
            r = requests.get(url=folder_feed, auth=auth, params={'maxhits': '10000'})
        ...
        document_request = requests.get(url=docs_url, auth=auth, params=docs_params)
        ...
        if case_doc_count < doc_count:
            sys.exit("Error:\tNot enough case documents to add to folders!\n"
                     ...
                     "\tHave: {case_doc_count} case documents\n\tExiting...".format(
                            doc_count=doc_count, case_doc_count=case_doc_count))
        ...
        with click.progressbar(etree.fromstring(r.text).findall('.//folder'),
            ...
            for folder in folders:
                ...
                t = threading.Thread(target=add_docs,
                                     args=(auth, host, port, case, doc_count, folder, max_threads, purge))
                t.start()
